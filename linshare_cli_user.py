...
section_server.add_element(Element('host', required=True,
...
section_server.add_element(Element('password', required=True, hidden=True,
    desc="user password to linshare",
    hooks=[Base64ElementHook(), ]))

section_server.add_element(Element('application_name', default="linshare",
    conf_hidden=True,
    desc="Default value is 'linshare' (example http:/x.x.x.x/linshare)"))

section_server.add_element(Element('nocache', e_type=bool, default=False,
...
parser.add_argument('-r', '--realm', action="store",
        **config.server.realm.get_arg_parse_arguments())

parser.add_argument('--nocache', action="store_true",
        **config.server.nocache.get_arg_parse_arguments())

parser.add_argument('-a', '--appname', action="store",
        **config.server.application_name.get_arg_parse_arguments())

# Adding all others parsers.
subparsers = parser.add_subparsers()
add_document_parser(subparsers, "documents",
                        "Documents management")
add_threads_parser(subparsers, "threads",
                        "threads management")
add_share_parser(subparsers, "shares",
                        "Created shares management")
add_received_share_parser(subparsers, "received_shares",
                        "Received shares management")
add_received_share_parser(subparsers, "rshares",
