...
class DomainFormatterResolver(Formatter):
    """Convert resource domain value to a readable name"""

    def __init__(self, cli, prop, full=False):
        super(DomainFormatterResolver, self).__init__(prop)
        self.full = full
        self.cli = cli
        self.domains = None
    ...
    def __call__(self, row, context=None):
        if self.domains is None:
            self.domains = {}
            for domain in self.cli.list():
                self.domains[domain['identifier']] = {
                    'label': domain['label'],
                    'type': domain['type'],
        ...
        parameter = row.get(self.prop)
        if parameter:
            l_format = '{label} ({identifier:.8})({type:.1})'
            if context.args.vertical or self.full:
                l_format = '{label} ({identifier})({type:.1})'
            row[self.prop] = l_format.format(**self.domains.get(parameter))
